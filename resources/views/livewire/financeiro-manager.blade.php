                    ...
                    <div class="sidebar-card-body" style="padding:8px 12px;">
                        @forelse ($this->proximosVencimentos as $v)
                            <div style="display:flex;align-items:center;justify-content:space-between;padding:8px 0;border-bottom:1px solid var(--border);font-size:12px;">
                                ...
                                <div style="text-align:right;"><span style="font-weight:700;color:var(--text);display:block;">R$ {{ number_format($v['valor'], 2, ',', '.') }}</span><span class="badge-sm {{ $v['status'] === 'pendente' && \Carbon\Carbon::parse($v['data_vencimento'])->isPast() ? 'badge-inativo' : ($v['status'] === 'pendente' ? 'badge-warning' : 'badge-ativo') }}" style="font-size:8px;">{{ \Carbon\Carbon::parse($v['data_vencimento'])->isPast() && $v['status'] === 'pendente' ? 'Vencido' : $v['status'] }}</span></div>
                            </div>
                        @empty
