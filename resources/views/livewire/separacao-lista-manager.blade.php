...
x-init="$watch('show', val => { if(val) setTimeout(() => show = false, 4000) })">
    ...
    <div style="max-width:480px;margin:0 auto;background:linear-gradient(180deg,color-mix(in srgb,var(--warning)5%,var(--surface)) 0%,var(--surface) 40%);min-height:100vh;">

        {{-- ===== HEADER ===== --}}
        <header style="background:linear-gradient(135deg,color-mix(in srgb,var(--warning)85%,#000),color-mix(in srgb,var(--warning)50%,var(--text)));color:var(--on-primary);padding:16px;display:flex;justify-content:space-between;align-items:center;position:sticky;top:0;z-index:20;box-shadow:0 2px 16px rgba(0,0,0,0.12);">
        ...
        <div style="padding:12px 16px;border-bottom:1px solid var(--border);background:var(--surface);">
            <div style="position:relative;display:flex;align-items:center;">
                ...
                <input wire:model.live.debounce.300ms="busca" placeholder="Buscar pedido, cliente ou ID..."
                       ...
                       @focus="$el.style.borderColor='var(--warning)'"
        ...
        <div style="padding:12px 16px;">
            ...
            @if (!empty($dados['atrasados']))
                <div style="margin-bottom:20px;">
                    <div style="display:flex;align-items:center;gap:8px;margin-bottom:10px;">
                        ...
                        <h3 style="font-size:10px;font-weight:800;color:var(--danger);text-transform:uppercase;letter-spacing:1px;margin:0;flex:1;">Atrasados</h3>
                        <span style="font-size:10px;font-weight:700;background:color-mix(in srgb,var(--danger)10%,transparent);color:var(--danger);padding:2px 8px;border-radius:10px;">{{ count($dados['atrasados']) }}</span>
                    </div>
            ...
            @if (!empty($dados['normais']))
                <div>
                    <div style="display:flex;align-items:center;gap:8px;margin-bottom:10px;">
                        ...
                        <h3 style="font-size:10px;font-weight:800;color:var(--warning);text-transform:uppercase;letter-spacing:1px;margin:0;flex:1;">Pendentes</h3>
                        <span style="font-size:10px;font-weight:700;background:color-mix(in srgb,var(--warning)10%,transparent);color:var(--warning);padding:2px 8px;border-radius:10px;">{{ count($dados['normais']) }}</span>
                    </div>
                    <div style="display:flex;flex-direction:column;gap:10px;">
            ...
            @if (empty($dados['atrasados']) && empty($dados['normais']))
                <div style="text-align:center;padding:60px 20px;">
                    <div style="width:80px;height:80px;border-radius:50%;background:color-mix(in srgb,var(--warning)8%,transparent);display:flex;align-items:center;justify-content:center;margin:0 auto 16px;">
                        <i class="fas fa-box-open" style="font-size:32px;color:var(--warning);opacity:0.5;"></i>
                    </div>
            ...
            @if ($dados['paginator']->hasPages())
                <div style="display:flex;align-items:center;justify-content:center;gap:4px;padding:20px 0;">
                    ...
                    @for ($page = $start; $page <= $end; $page++)
                        @if ($page == $current)
                            <span style="padding:6px 12px;border:0;border-radius:8px;background:var(--warning);color:#fff;font-size:11px;font-weight:800;box-shadow:0 2px 8px color-mix(in srgb,var(--warning)30%,transparent);">{{ $page }}</span>
                        @else
