    ...
    <div class="main-content-pad">
        ...
        <div class="metrics-grid" style="grid-template-columns:repeat(4,1fr);">
            ...
            <div class="metric-card" style="padding:14px;border-left:4px solid var(--warning);">
                <div><span class="metric-label">🟡 Atenção</span><span class="metric-value" style="font-size:20px;color:var(--warning);">{{ $resumo['media'] }}</span><span class="metric-sub">Vão acabar em breve</span></div>
            </div>
            <div class="metric-card" style="padding:14px;">
                <div><span class="metric-label">Sugestão Total</span><span class="metric-value" style="font-size:20px;">{{ number_format($resumo['sugestao_total'], 0, ',', '.') }} un</span><span class="metric-sub">Somar de todos os itens</span></div>
            </div>
            <div class="metric-card" style="padding:14px;">
                <div><span class="metric-label">Custo Total</span><span class="metric-value" style="font-size:20px;">R$ {{ number_format($resumo['custo_total'], 2, ',', '.') }}</span><span class="metric-sub">Investimento necessário</span></div>
            </div>
        </div>

        <div class="sub-card" style="margin-bottom:16px;display:flex;gap:10px;align-items:end;flex-wrap:wrap;">
            <div class="field" style="width:150px;margin:0;"><label>Loja *</label><select wire:model.live="lojaFiltro" style="height:38px;font-size:12px;">@foreach ($this->lojas as $l)<option value="{{ $l['id'] }}">{{ $l['nome'] }}</option>@endforeach</select></div>
            <div class="field" style="width:150px;margin:0;"><label>Departamento</label><select wire:model.live="categoriaFiltro" style="height:38px;font-size:12px;"><option value="">Todos</option>@foreach ($this->categorias as $c)<option value="{{ $c['nome'] }}">{{ $c['nome'] }}</option>@endforeach</select></div>
